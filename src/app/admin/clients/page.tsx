  ...
  const handleAdd = async () => {
    ...
    try {
      const res = await fetch('/api/clients', {
        ...
        body: JSON.stringify({ nameEn: newName.trim(), logo: newLogo || null, order: clients.length }),
      })
        ...
        <div className="flex flex-col gap-3">
          ...
          {newLogo && (
            <div className="flex items-center gap-3">
              ...
              <img src={newLogo} alt="preview" className="h-10 object-contain border rounded" />
        ...
        <div className="space-y-2">
          {clients.map(client => (
            <div key={client.id} className="flex items-center gap-4 bg-white border border-gray-200 rounded-xl px-4 py-3">
              ...
              {client.logo ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={client.logo} alt={client.nameEn} className="h-9 w-16 object-contain shrink-0" />
              ) : (
                <div className="h-9 w-16 bg-gray-100 rounded shrink-0" />
              )}
